...
- 日志历史查询、实时 tail、自动滚动、自动换行、全屏日志流。
- 支持自定义日期时间范围，并按 15 分钟、1 小时、6 小时或 24 小时时间片分批查询；最多合并 50000 行历史日志。
...
- 查询审计、导出审计。
- 查询时间范围、行数、并发限制。
- 当前批量历史查询最多在浏览器内存中保留 50000 行；单个时间片仍受 Loki `max_entries_limit` 限制。
- 日志脱敏和敏感字段保护。
- HTTPS、VPN 或统一身份认证。
